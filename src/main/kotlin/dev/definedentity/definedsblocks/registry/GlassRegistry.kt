...
object GlassRegistry {
    fun <T : Block> block(
        ...
        return REGISTRATE.block(name, blockSupplier).lang(englishName).properties({ p ->
            p.strength(0.3f).sound(SoundType.GLASS).noOcclusion()
        }).initialProperties(Material.GLASS).tag(BlockTags.IMPERMEABLE).simpleItem().recipe { ctx, prov ->
            prov.stonecutting(DataIngredient.stacks(Blocks.GLASS.asItem().defaultInstance)) { ctx.get() }
        }.register()
    }
