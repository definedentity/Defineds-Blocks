...
object FutureBlocks {
    val FUTURA_BLOCK = REGISTRATE.block(FutureBlock.ID, ::FutureBlock)
        .properties { p -> p.requiresCorrectToolForDrops().strength(1.5f, 6.0f) }.initialProperties(Material.STONE)
        .lang(TextFormatting.toEnglishName(FutureBlock.ID)).simpleItem().blockstate { ctx, prov ->
            prov.getVariantBuilder(ctx.get()).partialState()
                .with(FutureBlock.FUTURE_BLOCK_TYPE, FutureBlock.FutureBlockType.block).modelForState().modelFile(
                    prov.models().getExistingFile(DBIdentifier("block/futura_block/futura_block_online"))
                ).addModel().partialState().with(FutureBlock.FUTURE_BLOCK_TYPE, FutureBlock.FutureBlockType.column_x)
                ...
                .with(FutureBlock.FUTURE_BLOCK_TYPE, FutureBlock.FutureBlockType.column_y).modelForState().modelFile(
                    prov.models().getExistingFile(
                        DBIdentifier("block/futura_block/futura_block_column_online")
                    )
                ).addModel().partialState().with(FutureBlock.FUTURE_BLOCK_TYPE, FutureBlock.FutureBlockType.column_z)
                .modelForState().modelFile(
                    prov.models().getExistingFile(
                        DBIdentifier("block/futura_block/futura_block_column_online")
                    )
